# handwrite-scanner

📖 **[사용 안내서](https://bun3.github.io/handwrite-scanner/)** · ⬇ **[다운로드 (Releases)](https://github.com/Bun3/handwrite-scanner/releases)**

프린트된 양식에 손으로 작성한 문서 사진을 → 필드별 텍스트로 인식 → 검수 →
**검색 가능한 PDF**로 만들어주는 localhost 웹앱. 모든 처리(비전-LLM 포함)가
로컬 CPU에서 돌아가며 **데이터가 PC 밖으로 나가지 않는다**.

## 요구 사양

- Windows, RAM 16GB 권장 (8GB면 3B 모델로 교체 — `app/config.py`)
- GPU 불필요. 사무용 CPU 기준 문서 1장당 수 분 (배치로 걸어두고 나중에 검수)

## 설치 — 배포판 (권장, Python 불필요)

다운로드 페이지에서 **`handwrite-scanner-setup-버전.exe`**를 받아 실행하고 설치를 완료하세요. 압축 해제나 파일 덮어쓰기가 필요하지 않습니다. 시작 메뉴 또는 바탕화면의 **handwrite-scanner** 바로가기로 실행합니다.

기존 ZIP 버전 사용자는 먼저 프로그램 실행 창을 종료하고, 설치 위치로 **기존 handwrite-scanner.exe가 있는 폴더**를 선택하세요. 그 폴더의 모델·템플릿·작업 기록을 그대로 사용합니다. 새 폴더에 설치하면 별도 환경으로 시작합니다. 프로그램 제거 시에도 `data`와 `engine` 폴더는 보존됩니다.

- **개인용**: `handwrite-scanner.exe` 실행 → 첫 실행 시 엔진(~6GB) 자동 다운로드 → 브라우저 자동 열림
- **서버용**: 시작 메뉴의 **handwrite-scanner 서버 모드** 실행 → 콘솔에 표시되는 `http://<서버IP>:8000` 주소로
  다른 PC들이 브라우저 접속 (최초 1회 Windows 방화벽 허용 필요). 데이터는 사내망 밖으로 안 나감

## 설치 — 소스 (개발용)

```powershell
python -m venv .venv
.venv\Scripts\pip install -r requirements.txt
.venv\Scripts\python -c "from app.engine_setup import ensure_engine; ensure_engine()"  # llama.cpp + 모델 ~6GB
.\run.ps1   # 이후 브라우저에서 http://localhost:8000
```

배포판 빌드: Inno Setup 6 설치 후 `powershell -ExecutionPolicy Bypass -File build.ps1 -InnoCompiler "C:\경로\ISCC.exe"` → `dist\handwrite-scanner-setup-버전.exe` 및 자동 업데이트용 ZIP.

## 사용법

1. **템플릿 등록** (인식률의 핵심): 템플릿 메뉴 → 빈 양식 이미지/PDF 등록 →
   이미지 위에 드래그로 필드 박스를 그리고 라벨·타입 지정.
   - 타입: 자유텍스트 / 후보목록(예: 이름 명단) / 전화번호 / 숫자(범위 지정 가능) / 동그라미 선택
   - 후보목록·범위 제약을 걸수록 인식률이 크게 올라간다
2. **업로드·선택**: PDF/사진을 선택하면 팝업이 열립니다. 검사할 양식을 여러 개 체크하고, 스크롤로 미리보기를 확인하며 전체 선택·해제 또는 개별 체크로 페이지를 고른 뒤 **선택 적용**을 누르세요. 미리보기는 3열·5열·8열을 선택할 수 있고 작은 창에서는 열 수를 자동 조정합니다. 작업 화면에는 선택 수 요약만 남으며 **양식·페이지 선택**으로 다시 열 수 있습니다. 취소·Esc는 양식과 페이지 변경을 반영하지 않습니다. **인식 시작**을 누르면 선택한 양식과 일치하는 문서만 검사하고 나머지는 건너뜁니다. 미선택 페이지는 변환·인식하지 않으며, 검수와 표 내보내기에 원본 파일명·페이지 번호를 표시합니다. 임시 업로드는 24시간 후 다음 업로드 때 정리됩니다.
3. **검수**: 완료 후 필드별 crop 이미지와 인식 결과를 비교, 노란 강조(신뢰도 낮음)만 확인·수정
4. **다운로드**: PDF 2종 — 원본 모습 그대로 + 투명 텍스트층(검색/복사 가능), 또는 텍스트 재구성본

같은 브라우저 탭에서 화면을 이동하면 업로드가 끝난 입력의 양식·페이지 선택, 템플릿의 미저장 편집, 작업별 검수 필터·정렬·내보내기 설정을 복원합니다. 템플릿의 임시 편집은 **저장**해야 인식에 반영되며, **임시 변경 버리기**로 되돌릴 수 있습니다. 임시 보관은 탭을 닫으면 종료됩니다. 업로드 중에는 화면을 유지하고, 임시 입력이 만료됐다면 파일을 다시 선택하세요.

### 다른 PC로 옮기거나 나눠 처리하기

상단 **자료 이동**에서 저장된 템플릿과 작업을 골라 `.hscan` 파일로 내보내고 다른 PC에서 불러올 수 있습니다. 진행 중인 작업은 중단 후 내보내며, 받는 PC에서는 내용을 확인하고 등록한 뒤 **이어하기**를 누릅니다. 완료된 페이지와 검수 수정값은 유지되고, 중단 당시 처리 중이던 페이지는 처음부터 다시 인식합니다.

새 작업은 생성 시점의 양식과 기준 이미지를 전용 사본으로 보관합니다. 공용 템플릿을 수정해도 이어하기에는 영향을 주지 않습니다. 기존 작업을 처음 내보낼 때에는 현재 등록된 양식을 보관했다는 안내가 표시됩니다. 가져온 양식을 일반 템플릿 목록에도 추가할지 선택할 수 있으며, 이름이 같고 내용이 다르면 새 이름으로 등록합니다.

작업 하나를 선택하고 **페이지 분담**으로 미완료 페이지를 고르면, 맡긴 페이지는 이 PC의 이어하기에서 제외됩니다. 다른 PC에서 결과를 다시 내보내고 원래 PC에서 불러온 뒤 **기존 작업과 합치기**를 선택하세요. 충돌하는 페이지는 사용할 결과를 직접 선택하며, 두 원본을 보존한 새 통합 작업을 만듭니다. 이후 검색·통합 내보내기에 중복이 생기지 않도록 확인이 끝난 원본 작업의 보관 여부를 정리하세요. 전달을 취소했다면 **분담 해제**로 이 PC에서 다시 처리할 수 있습니다.

전체 작업 이동에는 보관 중인 업로드 원본도 포함됩니다. 페이지 분담 파일에는 선택한 페이지 이미지와 원본 파일명·페이지 번호만 포함됩니다. 모델 실행 파일·모델 본체, 미저장 브라우저 편집, 인식 시작 전 임시 입력은 포함하지 않습니다. 필요한 모델은 받는 PC에서 설치하거나, 확인 후 현재 모델로 남은 페이지를 처리할 수 있습니다. 자료 파일은 한 번에 압축 해제 기준 8GB, 개별 파일 2GB까지 지원합니다.

오류 또는 중단된 작업은 **이어하기**로 완료된 페이지의 결과를 유지하고 미완료 페이지부터 다시 처리할 수 있습니다. 처리 도중 멈춘 페이지는 그 페이지의 첫 항목부터 다시 인식합니다. **처음부터 재인식**은 기존 결과를 지우고 1페이지부터 다시 처리하므로, 템플릿이나 규칙을 바꾼 뒤 전체에 적용할 때 사용하세요.

**인식 시작**을 누르면 업로드 중 표시가 즉시 나타납니다. 파일 접수 후 PDF 변환은 백그라운드에서 진행하며, 작업 목록에서 문서 준비·엔진 준비·인식 상태를 확인할 수 있습니다. 오류가 발생하면 브라우저 알림과 작업 목록에 원인 및 해결 방법을 표시합니다. **기술 상세**를 펼치면 엔진 로그를 포함한 진단 내용을 복사할 수 있습니다. 저장 공간 부족으로 오류 상태조차 기록할 수 없을 때는 실행 중인 프로그램의 메모리에 안내를 유지합니다.

## 프로그램 업데이트

자동 업데이트 기능이 포함된 Windows 배포판에서는 새 버전 알림의 **지금 업데이트**를 누르면 다운로드·파일 검증·프로그램 교체·재실행을 자동으로 진행합니다. 실행 중인 작업과 모델 다운로드를 끝내고, 폰 업로드도 끈 뒤 진행하세요. 서버 모드라면 서버 PC의 브라우저에서 업데이트해야 합니다.

`data/`(템플릿·작업 기록·설정)와 `engine/`(모델)는 교체하지 않습니다. 새 버전 실행에 실패하면 이전 프로그램을 복원해 다시 실행합니다. 백업과 진단 기록은 `data/updates/`에 남으며, 전원 중단 등으로 도우미까지 종료된 경우에는 프로그램을 종료한 상태에서 해당 업데이트 폴더의 `recover.cmd`로 이전 프로그램 복구를 시도할 수 있습니다.

v0.7.0·v0.7.1처럼 이 기능이 없는 버전은 설치 EXE를 실행하고 기존 폴더를 선택해 전환할 수 있습니다. 개발용 Python 실행 환경은 자동 교체 대상이 아닙니다.

## 테스트

```powershell
.venv\Scripts\python -m pytest tests\test_postprocess.py     # 단위 테스트
.venv\Scripts\python -X utf8 -m pytest tests\test_e2e.py     # E2E (서버 기동 필요, 수 분)
```

## 구조


```
app/main.py        FastAPI 라우트 + 정적 프론트 서빙
app/llm.py         llama-server(Qwen3-VL-8B GGUF) 기동·호출
app/worker.py      인식 워커: 정합→필드 crop→VLM→후처리→2차 검증
app/postprocess.py 후보 유사도 매칭(자모 분해)·형식 검증·범위 보정
app/align.py       ORB+homography 로 사진을 템플릿 좌표계에 정합
app/pdf_gen.py     PDF 생성 (투명 텍스트층 / 텍스트 재구성)
data/templates/    템플릿(필드 좌표·라벨·후보) — 파일이 곧 상태, DB 없음
data/jobs/         작업별 입력·결과·PDF
```
