...
2. **업로드·선택**: PDF/사진을 선택 → 검사할 양식을 여러 개 체크 → 페이지 미리보기에서 전체 선택, 개별 체크 또는 범위(예: `1-5, 8, 12`) 지정 → 인식 시작. 선택한 양식과 일치하지 않는 처방전·증빙 서류 등은 인식하지 않고 건너뜁니다. 미선택 페이지는 변환·인식하지 않으며, 검수와 표 내보내기에 원본 파일명·페이지 번호를 표시합니다. 임시 업로드는 24시간 후 다음 업로드 때 정리됩니다.
