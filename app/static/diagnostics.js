...
(() => {
  ...
  function ensure() {
    ...
    dialog.innerHTML = `<h2 id="diagnosticTitle">오류 보고</h2>
      <p>앱 개발자에게 오류 코드·발생 위치·기기 사양·작업 상태와 최근 진단 기록을 보냅니다.
      ...
      <label>어떤 작업 중 발생했나요? (선택)<textarea id="diagnosticDescription" maxlength="4000" rows="3"></textarea></label>
    ...
    el('diagnosticPrepare').onclick = async () => {
      busy = true; sync();
      const data = {code, screen: location.pathname.split('/').pop().replace('.html', '') || 'index',
    ...
    el('diagnosticSave').onclick = () => {
      ...
      a.download = 'handwrite-error-report-' + new Date().toISOString().slice(0, 10) + '.json';
      a.click(); setTimeout(() => URL.revokeObjectURL(url), 10000);
    };
  }
  window.openDiagnosticReport = (errorCode = 'unexpected') => {
